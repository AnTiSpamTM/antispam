...
--Check members #اضافه supergroup
local function check_member_super(cb_extra, success, result)
  local receiver = cb_extra.receiver
  local data = cb_extra.data
  local msg = cb_extra.msg
  if success == 0 then
	send_large_msg(receiver, "❗️ابتدا مرا به ادمین ارتقاء دهید❗️")
  end
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '➕سوپرگروه به لیست اضافه شد➕'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end

--Check Members #حذف supergroup
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return '🔴لینک گزاشتن درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return '⚪️لینک گزاشتن درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️لینک باز شد⚪️'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "❗️این دستور برای مقام های بالا تر میباشد❗️"
  end
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == 'yes' then
    return '🔴اسپم درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '⚪️اسپم درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️اسپم باز شد⚪️'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return '🔴فلود درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return '⚪️فلود درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️فلود باز شد⚪️'
  end
end
...
local function anti_group_fosh(msg, data, target)
  ...
  local group_fosh_anti = data[tostring(target)]['settings']['anti_fosh']
  if group_fosh_anti == 'yes' then
    return '🔴فحاشی درحال حاظر قفل میباشد🔴'
  else
...
local function unanti_group_fosh(msg, data, target)
  ...
  if group_fosh_anti == 'no' then
    return '⚪️فحاشی درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['antifosh'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️فحاشی باز شد⚪️'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return '🔴عربی درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return '⚪️عربی درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️عربی باز شد⚪️'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return '🔴ممبر درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return '⚪️ممبر درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️ممبر باز شد⚪️'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return '🔴کاراکتر ار تی ال درحال حاضر قفل میباشد🔴'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔴کاراکتر ار تی ال قفل شد🔴'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return '⚪️کاراکتر ار تی ال درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️کاراکتر ار تی ال باز شد⚪️'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return '🔴سرویس تلگرام درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return '⚪️سرویس تلگرام درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️سرویس تلگرام باز شد⚪️'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return '🔴استیکر درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return '⚪️استیکر درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️استیکر باز شد⚪️'
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return '🔴شماره درحال حاضر قفل میباشد🔴'
  else
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return '⚪️شماره درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️شماره باز شد⚪️'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return '🔴تنظیمات سختگیرانه درحال حاضر قفل میباشد🔴'
  else
    data[tostring(target)]['settings']['strict'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔴تنظیمات سختگیرانه قفل شد🔴'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return '⚪️تنظیمات سختگیرانه درحال حاضر باز میباشد⚪️'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪تنظیمات سختگیرانه باز شد⚪️'
  end
end
...
local function lock_group_antiads(msg, data, target)
  ...
  if group_antiads_lock == 'yes' then
    return '🔴تبلبغات درحل حاظر قفل می باشد🔴'
  else
...
local function unlock_group_antiads(msg, data, target)
  ...
  if group_antiads_lock == 'no' then
    return '⚪️تبلیغات درحال حاظر باز می باشد⚪️'
  else
    data[tostring(target)]['settings']['lock_antiads'] = 'no'
    save_data(_config.moderation.data, data)
    return '⚪️تبلیغات باز شد⚪️'
  end
end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'اضافه' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '❗️این سوپرگروه درحال حاظر به لیست اضاقه شده است❗️', ok_cb, false)
			end
		...
		if matches[1] == 'حذف' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, '❗️سوپرگروه به لیست اضافه نشده است❗️', ok_cb, false)
			end
		...
		if matches[1] == 'لینک جدید' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '❗️خطا: ربات صاحب گروه نمیباشد❗️')
		...
		if matches[1] == 'ثبت لینک' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return '🔵لطفا لینک جدید را برای ثبت ارسال نمایید🔵'
		end
		...
		if msg.text then
			if msg.text:match("^(https://telegram.me/joinchat/%S+)$") and data[tostring(msg.to.id)]['settings']['set_link'] == 'waiting' and is_owner(msg) then
				...
				save_data(_config.moderation.data, data)
				return "✔️لینک جدید با موفقیت ثبت شد✔️"
			end
		end
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "🔵هنوز لینکی برای این گروه ثبت نشده است برای ثبت لینک از دستور (ثبت لینک) استفاده کنید🔵"
			end
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			if matches[2] == 'تنظیمات سختگیرانه' then
		...
		if matches[1] == 'با صدا' and is_momod(msg) then
			...
			local msg_type = 'صوت'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "⚪️ "..msg_type.." با صدا شد"
				else
					return "⚪️ "..msg_type.." با صدا است"
				end
			end
			if matches[2] == 'عکس' then
			local msg_type = 'عکس'
		...
		if matches[1] == "سایلنت" and is_momod(msg) then
			...
			elseif matches[1] == "سایلنت" and matches[2] and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] از ["..user_id.."] 🔶لیست بیصدا حذف شد")
					return "["..user_id.."] 🔷به لیست افراد بیصدا اضافه شد"
				elseif is_owner(msg) then
					mute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] به ["..user_id.."] 🔷لیست بیصدا اضافه شد")
					return "["..user_id.."] 🔶از لیست افراد بیصدا حذف شد"
				end
			elseif matches[1] == "سایلنت" and matches[2] and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'راهنما' and not is_owner(msg) then
			text = "❗️این دستور برای مقام های بالا تر است❗️"
